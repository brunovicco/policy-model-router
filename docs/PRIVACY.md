...
| `workflow_id` / `task_id` | Request body, caller-supplied | Echoed back in the decision record for the caller's own correlation | Contractual | Not persisted by this service; present only in the request/response of one call | N/A - not stored | N/A |
