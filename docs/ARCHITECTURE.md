...
│   ├── runtime_authorization.py  # Trusted key set, stable error, pure codec helpers (ADR-0012)
│   └── runtime_control.py        # Kill-switch state and snapshot (ADR-0014)
├── application/
...
│   ├── runtime_authorization_contract.py  # Pydantic mirror of the signed Governance envelope
│   ├── runtime_authorization.py           # Verifier + replay-guard port (ADR-0012)
│   └── runtime_control.py                 # Enforcer + projection-store port (ADR-0014)
├── adapters/
...
│   ├── runtime_authorization.py  # Replay guards, trusted key-set and agent-binding loading
│   └── runtime_control.py        # In-memory and Redis read-only projection stores
└── entrypoints/
...
Availability is resolved for the whole candidate set in one call, and a group the provider omits
is treated as unavailable (ADR-0006's second amendment).
...
|---|---|---|
| No live availability signal | `AvailabilityProvider` (ADR-0006, amended to be `async` and then to resolve the whole candidate set in one call) is a real seam, but the only shipped implementation (`StaticAvailabilityProvider`) still just passes through the static YAML flag; nothing polls provider/gateway health | A group can be selected while its actual deployments are down; the policy file must be edited and the service redeployed to reflect an outage. Not resolved: no real health-check target exists yet to poll - adding one now would mean integrating against a system that isn't there |
...
estimated input/output token counts. Both rate-limit tiers previously sat inside the `/route`
